...
FILE * fdebug;		/* file for debugging strings */
...
{	int i,j,lev,t, ty;
	...
	ftemp = fopen( (tempname = mktemp(TEMPNAME)), "w" );
	faction = fopen( (actname = mktemp(ACTNAME)), "w" );
	...
	cnamp = cnames;
	defin(0,"$end");
	extval = 0400;	/* this junky number is known where fdebug is used */
	defin(0,"error");
	defin(1,"$accept");
	mem=mem0;
	lev = 0;
	...
	defout(1);
		 ...
		 * PORT: long, not int.
		 *
		 * YYSTYPE is the parser's value stack, and a grammar with no
		 * %union puts whatever it likes on it.  V8's grammars put
		 * POINTERS there -- eqn pushes the char * it has just lexed,
		 * pic and grap do the same -- which was free on the VAX, where
		 * an int and a pointer were both 32 bits.
		 *
		 * Under LP64 an int stack truncates every one of them.  eqn
		 * died in lookup() scanning a name whose top word was gone:
		 *
		 *	lk 0000000100b75c00     from the lexer, intact
		 *	lk 0000000000b75c00     the same string off the stack
		 *
		 * Fixing it here rather than per grammar is the whole point:
		 * every yacc-built program in the tree -- eqn, pic, grap,
		 * ideal, expr, make, awk -- gets it from this one line, and a
		 * grammar that declares its own %union still overrides it.
		 */
	...
	more_rule:
		...
		if( t == '=' ){
			...
			if( (t=gettok()) == IDENTIFIER ){
				...
				sprintf( actnm, "$$%d", nprod );
				j = chfind(1,actnm);  /* make it a nonterminal */
...
defout(last) /* write out the defines (at the end of the declaration section) */
{	register int i, c;
	register char *cp;
	static int beenhere = 0;

	if(!beenhere++ && fdebug != NULL)
		for(i = 0; i < sizeof(tmpx)/sizeof(tmpx[0]); tmpx[i++] = -1)
			;
	...
	ndefout = ntokens+1;
	if(last && fdebug != NULL) {
		fprintf(fdebug, "char *yytoknames[] = {\n");
...
begin:
	...
	default:
		...
		if( isdigit(c) ){ /* number */
			...
			i = 0;
			while( islower(c) || isupper(c) || isdigit(c) || c == '-' || c=='_' || c=='.' || c=='$' ){
				tokname[i] = c;
...
openup(stem, Dflag, dflag, vflag, ytab, ytabc)
	char *stem, *ytabc;
{
	char buf[256];
	...
	if(Dflag)
	{
		sprintf(buf, "%s.%s", stem, FILEDEBUG);
		if((fdebug = fopen(buf, "w")) == NULL)
			error("can't open %s", buf);
		fprintf(fdebug, "#ifdef YYDEBUG\n");
	}
	if(dflag)
	{
		sprintf(buf, "%s.%s", stem, FILED);
		fdefine = fopen( buf, "w" );
		if(fdefine == NULL)
			error("can't create %s", buf);
	}
	if(ytab == 0)
		sprintf(buf, "%s.%s", stem, OFILE);
	else
		strcpy(buf, ytabc);
	ftable = fopen( buf, "w" );
	if( ftable == NULL ) error( "cannot open table file %s", buf);
}
