...
char	a;
{
	...
	/*
	 * PORT: /tmp, not /usr/tmp.
	 *
	 * /usr/tmp was a link to /var/tmp on the systems this ran on; macOS has
	 * neither, and /usr is protected by SIP so it cannot be made.  /tmp is
	 * the same thing under the name that survived.
	 */
	p = mktemp("/tmp/trtmpXXXXX");
	if (a == 'a')
		p = &p[9];
...
fdprintf(fd, fmt, x1)
	int	fd;
	register char	*fmt;
	unsigned long	x1;	/* PORT: one whole argument slot; see below */
{
	register c;
	/*
	 * PORT: unsigned long, not unsigned int.
	 *
	 * This is V8's varargs idiom -- take the address of the last named
	 * parameter and walk forward -- and the stride has to match the width of
	 * an argument slot.  SZARG is SZLONG here (see compiler/ccom-arm64/
	 * macdefs.h), because the prologue spills x0-x7 into one contiguous
	 * 8-byte-per-slot block and that is what makes &arg walking work at all.
	 * With a 4-byte stride every argument after the first was read from the
	 * wrong half of a slot, and `%s` produced a pointer with its top 32 bits
	 * missing: nroff died on its first message reading 0x6fdfe460, which is
	 * a stack address short of its high word.
	 *
	 * The `%D` and `%O` cases below dropped their extra `adx +=` for the same
	 * reason: a long now occupies exactly one slot, like everything else.
	 */
