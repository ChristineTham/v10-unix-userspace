...
#	define	NBLIST	1024
#else
#	define	NBLIST	2048	/* allocation list.  smallish machines use 1024 */
#endif
 ...
 * PORT: int, not long.
 *
 * tdef.h's own comment two hundred lines up calls this "a 32 bit cookie", and
 * troff means it: a tchar packs a character code with its size and font using
 * the explicit masks below, and n7.c reads and writes the top bit by hand.  On
 * the VAX a long was exactly the 32 bits described.  Under LP64 it is 64, which
 * both changes the packing arithmetic and makes `tchar gettch()` conflict with
 * the implicit-int declarations elsewhere in the tree -- nroff would not compile
 * at all.
 *
 * int is the type that is 32 bits here, so int is what the comment asks for.
 */
typedef	int	tchar;
