/*
 * PORT: 128, not V7's 32, matching FSNMLG in <fstab.h> -- see the account
 ...
 * tell the next reader the record is 64 bytes when it is 256.  That is the
 * DIRSIZ failure -- three headers, two patched, the unpatched one still
 * believed.
 */
struct mtab {
	char	m_path[128];		/* mounted on pathname */
	char	m_dname[128];		/* block device pathname */
};
