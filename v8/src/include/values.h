 ...
 */
/* PORT: a fourth machine arm.  values.h describes a floating-point format and
   selects one by a macro the compiler predefines; upstream's three are u3b,
   vax and gcos, and this port's cc predefines `unix' and `arm64', so on this
   target NONE of them is taken and _DEXPLEN, _HIDDENBIT and _IEEE are simply
   undefined.  The failure is loud -- libI77/ecvt.c, the tree's only includer,
   would not compile -- rather than a wrong number, which is the good
   direction.  The values are IEEE 754 binary64/binary32, identical to the u3b
   arm below because the 3B20 was IEEE too; they are stated separately anyway,
   because sharing the arm would make this machine claim to be that one.
   src/include/PORTING.md. */
#if arm64
...
/* PORT: and DMAXPOWTWO cannot use the generic formula at the bottom of this
   file, because that formula assumes the mantissa is WIDER THAN A LONG.  It is
   (double)(1L << BITS(long)-2) * (1L << DSIGNIF-BITS(long)+1), which needs
   DSIGNIF > BITS(long) for the second shift to be non-negative: on a VAX that
   is 56 > 32 and the shift is 25, and under LP64 it is 53 < 64 and the shift
   is MINUS TEN -- undefined behaviour, evaluated at run time by ecvt.c:64.
   The quantity meant is the largest double whose neighbours are one apart,
   i.e. 2^DSIGNIF, which is directly expressible here precisely because DSIGNIF
   is smaller than a long rather than larger. */
#define DMAXPOWTWO	((double)(1L << DSIGNIF))
...
/* PORT: guarded, so the arm64 arm above can state its own.  Upstream writes
   these unconditionally because every machine it knows has DSIGNIF > BITS(long)
   and the one formula serves them all; see the note in the arm64 arm for why
   LP64 breaks that.  A guard rather than a fourth `#if' here, because what has
   to be said is "this machine already answered", which is what #ifndef says. */
#ifndef DMAXPOWTWO
