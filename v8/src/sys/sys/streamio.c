...
caddr_t arg;
{
	...
	case FIOINSLD:
		...
		if (qattach(streamtab[ld.ld], RD(q), ip->i_un.i_rdev)) {
			...
			if (nip==0) {
				...
				panic("pushld qopen returns inode", nip);	
			splx(s);
			goto out;
		}
		splx(s);
	...
	case FIOLOOKLD:
		...
		if (arg) {
			/*
			 * PORT: upstream is `sizeof(arg)'.  arg is caddr_t
			 * (:543) and fmt is int (:549), so on LP64 this reads
			 * EIGHT bytes out of a FOUR-byte object and writes
			 * them to user memory.  On the VAX both were 4, so
			 * the line computed the right number by coincidence.
			 *
			 * A typo rather than a design, and argued from eight
			 * controls: stioctl copies at :562 :567 :576 :623
			 * :665 :690 :713 :732 :779, and every one but this
			 * names the OBJECT -- nld, stq->pgrp, ld, a
			 * sizeof(union stmsg), or a computed wptr-rptr.  One
			 * of nine names the pointer.
			 *
			 * Reached by FIOLOOKLD with a non-null argument.
			 * src/sys/PORTING.md hazard 1.
			 */
			if (copyout((caddr_t)&fmt, arg, sizeof(fmt))) {
				u.u_error = EINVAL;
				goto out;
			}
		} else
...
urcvfile(stq, arg)
register struct stdata *stq;
caddr_t arg;		/* PORT: upstream declares only stq, so arg is an
			 * implicit int.  stioctl:614 calls urcvfile(stq, arg)
			 * with its own `caddr_t arg' (:543), so on LP64 the
			 * user address is TRUNCATED to 32 bits on entry and
			 * sign-extended again at the copyout below -- a wild
			 * pointer write, from FIORCVFD.  On the VAX an int and
			 * a pointer were both four bytes and the omission cost
			 * nothing.
			 *
			 * The twin one function up settles it: usndfile(stq,
			 * arg) at :930 declares `caddr_t arg' for the same
			 * argument from the same caller.  One of the pair has
			 * it and the other does not, which is an omission
			 * rather than a design -- the same shape as
			 * unexpand.c missing the argc guard that expand.c has
			 * beside it.  src/sys/PORTING.md, "Two deviations". */
{
	register struct block *bp;
	register i;
	register struct file *fp;
