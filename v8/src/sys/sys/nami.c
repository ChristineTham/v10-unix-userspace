...
#if	DIRSIZ == 14
			/*
			 * PORT: upstream is `*(long *)' in all three.  The
			 * offsets 0, 4, 8 and 12 are the whole argument: this
			 * is a hand-unrolled 14-byte compare, and 4+4+4+2 is
			 * 14 ONLY BECAUSE V8's long is 32 bits.  That is
			 * cmd/ccom/vax/macdefs.h:20, `# define NOLONG', "map
			 * longs to ints" -- the same one line that made every
			 * on-disk struct in this port the wrong size (§8a
			 * step 4a, src/include/PORTING.md).  The `#if' one
			 * line above is upstream stating the arithmetic it
			 * depends on, and stating it in the one term that
			 * does not travel.
			 *
			 * Here long is 8, so the three compares read 24 bytes
			 * where 12 were meant and the fourth reads 2 more.
			 * d_name is 14 and struct direct is 16, so bytes 14
			 * and 15 of THIS entry and the first ten of the NEXT
			 * decide an equality that is supposed to be about the
			 * name -- and the last entry in a block reads past
			 * the buffer.  dsearch() then fails to match a name
			 * that is present, so namei() returns ENOENT for
			 * every path and no file can be opened at all.
			 *
			 * int is 32 bits here, so it reproduces the VAX
			 * exactly.  short is 2 on both and is left alone.
			 *
			 * THE WRONG FIX IS -DDIRSIZ=254, which reaches the
			 * strncmp arm below and makes the symptom vanish by
			 * changing the size of an on-disk record.  DIRSIZ is
			 * 14 in src/sys/ because that is what a V8 disk holds
			 * -- CLAUDE.md, "one number per layer".
			 *
			 * src/sys/PORTING.md, "nami.c: the name compare".
			 */
			if (*(int *)&nm[0] == *(int *)&dp->d_name[0] &&
			    *(int *)&nm[4] == *(int *)&dp->d_name[4] &&
			    *(int *)&nm[8] == *(int *)&dp->d_name[8] &&
