 ...
 * copied from jerq/include (see the Makefile).  FOURTH instance of the
 * identical change, after src/cmd/ls.c:11, src/cmd/mc.c and
 * src/lib/libtermlib/termcap.c.
 *
 * The block is LIVE here, as it is in mc.c and unlike ls.c and termcap.c: p.c's
 * FIRST LINE is `#define JERQ  No home should be without one', so the file
 * turns it on itself.  ioctl(1, JWINSIZE) still fails -- the shim implements
 * no JWINSIZE -- so ismpx stays 0 and p keeps DEF and WIDTH, which are
 * upstream's own fallbacks.
 */
#include <jioctl.h>
int	ismpx=0;
#endif
