...
char *argv[];
{
	...
	/* PORT: `argv[1]==0 ?' and `argv[1]!=0 &&' added.  main() has no argc
	 * check at all, so a bare `mkstand' reads *argv[1] -- the argument
	 * vector's NULL terminator -- as its FIRST statement.  A VAX put the
	 * text segment at virtual 0 and read the first byte of crt0 there,
	 * which this port has measured as 0x00, so `number' became 0 and then
	 * 0-'0'; macOS leaves page 0 unmapped and it is SIGSEGV.  The guards
	 * restore the VAX's ANSWER rather than merely removing the fault, which
	 * is why they yield 0 and a false isdigit() instead of returning early.
	 * Both reads, because argv[1]+1 is address 1 and crt0's second byte is
	 * 0x00 as well.  Deliberately NOT guarded: `pid=argv[2]' three lines
	 * below reads PAST the terminator into the environment vector, which is
	 * a valid read on both machines, and the sprintf into `char name[14]'
	 * that follows overruns it identically on a VAX.  That is upstream's
	 * defect on upstream's hardware and S1 leaves it alone. */
	number = argv[1] == 0 ? 0 : *argv[1];
	number=number-'0';
	if(argv[1] != 0 && isdigit(*(argv[1]+1)))number=10*number+(*(argv[1]+1)-'0');
	orignum=number;
	pid=argv[2];
