...
int f; register struct proc *pp; register struct user *up;
{
	...
	/*
	 * PORT: long, not int.  nstack is ctob(up->u_ssize) and u_ssize is
	 * size_t -- 8 bytes here, 4 on the VAX -- so the shift happens at 64
	 * bits and the store narrows to 32.  The `if (nstack > NSTACK)' clamp
	 * below is AFTER that, so a large u_ssize could give nstack == 0, and
	 * the scan then reads stack[-1] before its own guard can stop it.
	 * shim/libkmemu/procfs.c picks u_ssize partly to avoid that; this makes
	 * the arithmetic right rather than relying on the choice.
	 */
	long nstack, staddr, stblk;
