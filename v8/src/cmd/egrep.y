...
yylex() {
	/*
	 * PORT: upstream `extern int yylval;'.  yacc DEFINES yylval, and V8's
	 * yacc as patched here emits `#define YYSTYPE long' for an untyped
	 * grammar (src/cmd/yacc/y2.c, the ntypes==0 arm) -- so the object is
	 * eight bytes and this declaration described four.  Third instance of
	 * the class after awk.lx.l and ratfor's r.h.
	 *
	 * AND IT IS THE ONE THAT ANNOUNCED ITSELF, because here the
	 * declaration and the definition end up in ONE translation unit: yacc
	 * writes both into y.tab.c and v8cc refuses with `redeclaration of
	 * yylval from some line 76'.  In awk and ratfor the lexer is a
	 * separate file and nothing could speak.  Nothing here was silently
	 * wrong either: the only store is `yylval = c' below and every
	 * semantic value is a character or a node index -- enter(), node(),
	 * unary() and cclenter() all return the subscript `line' -- so unlike
	 * awk and ratfor no pointer passes through it.  The change is forced
	 * by the build, not by an observed wrong answer.
	 */
	extern long yylval;
	int cclcnt, x;
	register char c, d;
