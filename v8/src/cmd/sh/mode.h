 ...
 * PORT: long, not int.
 *
 * These are the shell's own pointer/integer puns, and mode.h exists to hold
 * exactly this kind of machine assumption -- the file already switches BOOL on
 * `#ifdef pdp11` two declarations above.  On the VAX an int held a pointer
 * exactly; under LP64 it holds half of one.
 *
 * The compiler cannot rescue this the way it does an implicit conversion.
 * addblok() computes
 *
 *	bloktop = (struct blk *)(Rcheat(bloktop) + reqd);
 *
 * with reqd unsigned, so once the cast says `int` the ADDITION is 32-bit
 * arithmetic and C is right to truncate it.  The shell built its arena at
 * 0x1057d0000, stored 0x057d0200 back, and then hung -- the store faulted, the
 * signal handler stdsigs() had just installed returned, and the faulting
