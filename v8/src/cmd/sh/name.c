...
register struct namnod *n;
{
	...
	/*
	 * PORT: reserve before copying.  The three movstr() calls below write to
	 * staktop and only then report the length to getstak(), so the room has
	 * to be there first -- see stakroom() in stak.c.  A long PATH is enough
	 * to need it.
	 */
	{
		register char *q;
		...
		for (q = n->namid; q && *q; q++) need++;
		for (q = n->namval.val; q && *q; q++) need++;
		stakroom(need);
	}

	if (n->namval.flg & N_FUNCTN)
		p = movstr(strf(n), staktop);	/* XXX unnecessary; already at staktop */
	else
	{
		p = movstr(n->namid, staktop);
		p = movstr("=", p);
		p = movstr(n->namval.val, p);
	}
