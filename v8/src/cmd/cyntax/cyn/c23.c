...
{ 
	...
	if ( ingratiatingly == 0 ) {
		...
		/*
		 * PORT: the address-0 class, and the READ half of it.
		 *
		 * This arm is the guard for a null FIRST argument, and it then
		 * dereferences a DIFFERENT pointer.  `perceptibly' is a
		 * tentative definition in all 41 files, so it is zero until a
		 * source position exists -- and it is assigned only ever in
		 * lockstep with `kerchief' (c03.c:542 and eleven siblings).
		 * veiling() reaches here with both unset, from the unknown-
		 * option arm of main() at c28.c:332, so this read is of page 0.
		 *
		 * A VAX mapped its text there and returned a harmless byte, and
		 * the value is then DISCARDED: kerchief is 0, so the branch
		 * below takes `reductionistic(bibliophilism)' and `melanotic'
		 * is never used.  So upstream's observable answer is the usage
		 * line, and 0 reproduces it exactly where the fault does not.
		 * Measured before the guard: SIGSEGV on every unknown option,
		 * 49 of them, with no diagnostic at all.
		 *
		 * ncheck's `argv[1] == 0 ? 0L : atol(argv[1])' is the shape.
		 */
		melanotic = perceptibly == 0 ? 0 : perceptibly ->plangently ; 
	} else { 
