 ...
 */
/*
 * PORT: the world's headers.
 *
 * V8 needed no such thing because /usr/include WAS the system's, and cpp
 * hardcodes it (cpp.c:1119).  Ours lives under $V8ROOT, and the installed
 * /lib/cpp is still the clang-built stage-0 binary, so it never sees
 * rootpath() and cannot find the jail's headers on its own -- every #include
 * came back "Can't find include file".  src/cmd/cc.c:112 carries the identical
 * line for the identical reason and states it at cc.c:406-409; this is that
 * decision applied to the second program in the tree that drives cpp directly.
 *
 * Appended to cpp_args after options() has run, so a user's own -I still takes
 * precedence -- the ordering cc.c chose.
 */
char		*incdir;
...
uinit()
{
	...
	/*
	 * PORT: $V8ROOT, then the root compiled in at build time -- the same
	 * decision v8root() makes in shim/v8sys/syscall.c and setpaths() makes
	 * in cc.c.  See the note on incdir above.
	 */
	...
	if (r != NULL && *r != '\0')
	{
		incdir = salloc(strlen(r) + sizeof "-I/usr/include");
		sprintf(incdir, "-I%s/usr/include", r);
	}
}
...
{
	/*
	 * PORT: after options(), so a user's -I comes first.  See incdir above.
	 */
	if (incdir != NULL)
		add_arg(incdir, cpp_args);
