...
#ifdef arm64
/*
 * PORT: MAXDATA IS A CEILING ADDRESS, AND THE ONLY MACHINE-DEPENDENT LINE IN
 * THIS PROGRAM.  main() reads
 *
 *	memsize = (MEMSIZE)sbrk(0);
 *	memsize = MAXDATA - memsize - sizeof(int);
 *
 * -- the top of the data segment less how far the break has already got, which
 * is an arena size only where the data segment starts near zero.  It does on a
 * VAX and a pdp11; on Mach-O/arm64 the break is wherever dyld left it, several
 * gigabytes up, and MEMSIZE is `unsigned' (32 bits under LP64), so the cast
 * truncates an address and the subtraction underflows.  Same family as V8
 * assuming address 0 is readable.
 *
 * So this arm names the arena and derives the ceiling, which makes the two
 * lines above compute 400K-4 exactly.  The truncation in the source is then
 * harmless rather than merely tolerated: both sbrk(0) values are truncated the
 * same way and the difference is exact mod 2^32.  Written here rather than as
 * an edit to main() for the reason shim/kern/h/param.h redirects printf --
 * a definition can be machine-dependent where a rewritten statement is just a
 * rewritten statement.
 *
 * The MAGNITUDE is deliberately the VAX's and not a modern one.  Only the base
 * of the arithmetic is forced by the target; enlarging the arena would be
 * taste, and it would cost something real -- fsck's scratch-file path (the
 * `NEED SCRATCH FILE' branch in makefs()) is authentic code that runs only
 * when the maps do not fit, and an 8 MB arena would make it unreachable for
 * any image this port can produce.
 */
...
char	pathname[200];
/*
 * PORT: 1024, not V7's 80.  The -t arm in main() copies its argument in with
 * an unbounded `while(*p++ = **argv)', so a scratch-file path longer than
 * eighty characters runs off the end of this array into whatever the linker
 * put next in __DATA,__common -- lfname, checklist, big, lncntp.  Same class
 * as rmdir's name[500] and mkdir's pname[128], and reachable from the command
 * line with no filesystem involved at all.  1024 is macOS's PATH_MAX, the
 * longest argument the kernel will hand this program.
 *
 * Note pathname[200] above is NOT this bug, and the difference is instructive:
 * pass2() copies at most DIRSIZ bytes per component into it, so its size is a
 * sentence about DIRSIZ -- 200 bytes is thirteen levels of 14-character names.
 * It is correct here only because fsck is in the Makefile's $(IMGBIN) group and
 * therefore compiled -DDIRSIZ=14.  At this port's host DIRSIZ of 254 a SINGLE
 * component would overrun it by 54 bytes.  That makes fsck the one member of
 * the group where forgetting the flag corrupts memory rather than an answer.
 */
char	scrfile[1024];
	...
	while(--argc > 0 && **++argv == '-') {
		switch(*++*argv) {
			...
			case 'T':
				 ...
				 *	while(*p++ = **argv) (*argv)++;
				 * See the note on scrfile[]'s declaration.
				 * Enlarging the array alone would not do:
				 * PATH_MAX is what the kernel accepts as one
				 * argument, and argv[] can carry more than
				 * that.  Both exits leave a NUL.
				 */
				p = scrfile;
				while(p < &scrfile[sizeof(scrfile)-1] &&
				      (*p++ = **argv))
					(*argv)++;
				*p = 0;
				break;
...
{
	/*
	 * PORT: 1024+2, not V7's 32.  Three unbounded copies below build
	 * "/dev" + "/r" + "disk4" out of a caller's path, and 32 bytes was
	 * sized for /dev/rp0.  The argument is whatever was on the command
	 * line, and it only has to STAT as a block device -- it does not have
	 * to be short or to live in /dev -- so this is reachable in the one
	 * program here that writes to filesystems.  1024 is macOS's PATH_MAX,
	 * and the +2 is the 'r' rawname() inserts plus the NUL: the longest
	 * input string is 1023, so the longest output is 1024 and it needs
	 * 1025 bytes.  Written as a sum so the reason stays attached.
	 */
	static char rawbuf[1024+2];
	char *dp = rindex(cp, '/');
...
char *dev;
{
	...
	daddr_t blk;
	BUFAREA *bp1, *bp2;
	time_t nowtime;		/* PORT: see the note below, and pinode() */
	...
	if(dfile.mod) {
		/*
		 * PORT: upstream is `time(&superblk.s_time)'.  s_time is a
		 * DISK field, narrowed to int with the rest of struct filsys
		 * so the superblock is the VAX's 4096 bytes; time() writes a
		 * time_t, which is eight here, so four bytes land on s_tfree
		 * at offset 220.  Second instance -- icheck.c:544 was the
		 * first, and this one is WORSE: icheck assigns s_tfree = 0
		 * two lines later, and fsck calls sbdirty() and writes the
		 * superblock to disk.  A repaired filesystem would come back
		 * claiming zero free blocks.
		 */
		time(&nowtime);
		superblk.s_time = nowtime;
		sbdirty();
	}
	ckfini();
	sync();
...
pinode()
{
	...
	/*
	 * PORT: upstream is `p = ctime(&dp->di_mtime)'.  THE READ DIRECTION OF
	 * THE SAME SEAM as check1()'s time() above, and it is the one that
	 * hangs.  di_mtime is a disk field, four bytes; ctime() takes a time_t
	 * pointer and dereferences eight, so it reads di_mtime as the low half
	 * and di_ctime as the HIGH half -- about 7.3e18 seconds.  gmtime()'s
	 * `while(days >= dysize(y)) y++' then counts to the year 2.3e11 one
	 * year at a time.  Not a wrong string: a live lock, with nothing on
	 * stdout because it never flushes.
	 *
	 * Swept: this is the only ctime/localtime/gmtime call in the tree whose
	 * argument is narrower than a time_t; the other seventeen take a
	 * time_t, a long, or a host struct stat's st_mtime.  icheck's fix swept
	 * `time(&' -- the WRITE direction -- and was correct for the tree as it
	 * then stood.  Importing one program falsified it, which is the general
	 * shape here: a sweep is true of a tree at a moment.
	 */
	mtime = dp->di_mtime;
	p = ctime(&mtime);
