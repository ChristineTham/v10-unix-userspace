...
{
	register i,m,n;
	/* PORT: `argv[1]!=0 &&' added.  This is main's FIRST statement, so a
	 * bare `diff3' dereferences the vector's NULL terminator before
	 * anything is checked -- byte for byte diffh.c:93's shape, and the
	 * ninth instance of the class (PLAN.md S4i).
	 *
	 * Only the bare invocation is affected, unlike diffh's: this is an `if'
	 * rather than a loop, so it runs once and `diff3 -x' still has an
	 * argv[1] to read.
	 *
	 * On the VAX address 0 held crt0's first byte, 0x00 -- V8's binaries
	 * are ZMAGIC, so N_TXTOFF is 1024 and the a.out header is never mapped
	 * -- which is not '-', so the test simply failed and the argc<6 check
	 * below printed `diff3: arg count' and exited 1.  That answer is what
	 * the guard restores; macOS leaves page 0 unmapped, so the same read is
	 * a SIGSEGV before a word of diagnostic. */
	if(argv[1]!=0 && *argv[1]=='-') {
		switch(argv[1][1]) {
		default:
			eflag = 3;
