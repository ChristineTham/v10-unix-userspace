...
/* PORT: THE CELL IS ONE MACHINE WORD, AND THE WORD GREW.  b_space is bc's
   arena of compiled dc fragments and every cell holds a POINTER -- either a
   char * literal ("S", "s.", getf()'s two-byte name out of funtab) or a long *
   to a nested bundle, which is how routput() tells them apart -- it asks
   whether the value lies inside b_space.  (No line number: the citation would
   sit in the file it cites, which is how the last four in this tree went stale
   the moment the comment above them grew.)  Upstream spells the cell
   `int' because a VAX pointer was four bytes.  Here it is eight, so an int
   cell truncates every fragment address it is handed.
   Widen the TYPE, not the uses -- struct(1)'s VERT, and efl's `typedef int
   *ptr' from the other side.  The declarations below cascade from this line
   and nothing else in the file changes.  */
long b_space [ b_sp_max ];
long * b_sp_nxt = { b_space };

int	bdebug = 0;
long *
/* PORT: ...AND THE SECOND HALF OF THE SAME FACT IS THE ARGUMENT WALK.
   `p = &a; i = *p++;' takes the address of the first K&R parameter and strides
   forward through the rest -- exact on a VAX, where arguments sat four bytes
   apart, and wrong here because v8cc spills x0-x7 into EIGHT-byte slots
   (SZARG is SZLONG).  This is the forward form of the idiom libc already
   walks with an eight-byte type: doprnt.c's NEXTLONG, and fprintf.c handing
   it &args.  CLAUDE.md records the indexed form as a singleton in mkfs's
   gmode(); it is not, and this is the second.
   Declaring `a' long is what makes the stride eight -- one change, and it
   fixes the truncation above at the same time, because a long cell holds a
   whole pointer.  Measured before: bc emitted nothing but `q' for every
   input, because the first cell read was the HIGH half of the count's slot,
   which is zero, and routput's `while (*p != 0)' stopped at once.  */
bundle(a) long a; {
	long i, *p, *q;
