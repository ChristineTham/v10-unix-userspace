...
/*
 * PORT: four ABSOLUTE includes into the kernel source tree become <sys/...>.
 * Upstream writes "/usr/sys/h/param.h" and three more like it, and this port
 * has no /usr/sys.  Same change ls.c:11 and mc.c already carry for
 * "/usr/jerq/include/jioctl.h".
 *
 * IT IS A MEASURED NO-OP RATHER THAN AN EQUIVALENCE ARGUED FROM THE NAMES.  On
 * the shipped V8 the kernel headers are installed into /usr/include/sys too,
 * and all four pairs are BYTE-IDENTICAL -- cmp usr/include/sys/param.h against
 * usr/sys/h/param.h and so on for stream.h, inode.h and conf.h.  So the two
 * spellings named one file, and only param.h differs HERE, because this port
 * patches its own (DIRSIZ 254 and the #ifndef guard).  showq reads no
 * directories, so that difference cannot reach it.
 */
#include <nlist.h>
#include <sys/param.h>
#include <ctype.h>
