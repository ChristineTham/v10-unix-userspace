...
{
	/*
	 * PORT: `long', was an implicit-int `extern savint;'.
	 *
	 * The SAME VARIABLE IS DECLARED IN TWO FILES and only one of them is its
	 * definition (getfile.c:124).  Widening that alone would have left this
	 * translation unit reading the low four bytes of an eight-byte object --
	 * silently, on a little-endian machine, and correctly for every value
	 * below 2^31.  The comparison below is the sentinel test, so it would have
	 * kept working while signal() got half an address.
	 */
	extern long savint;	/* error during a ! > < | ?? */
	unlock();
	if(code){
