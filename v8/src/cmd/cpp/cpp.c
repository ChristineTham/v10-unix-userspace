...
STATIC	FILE	*fout;		/* = stdout; set in main -- see note below */
STATIC	int	nd	= 1;
...
yyerror(s,a,b) char *s; char *a, *b; {
	pperror(s,a,b);
}

ppwarn(s,x) char *s; char *x; {
	int fail = exfail;
	exfail = -1;
	pperror(s,x);
	exfail = fail;
	...
	char *argv[];
{
	...
	register char *p;
	char *tf,**cp2;
	fout = stdout;	/* not a link-time constant under a hosted libc */
	setbuf(stderr, stbuf);
# if gcos
