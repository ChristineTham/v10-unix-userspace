...
typedef struct rrow {
	int ltype;
	/*
	 * PORT: upstream `int lval'.  It holds either a character value or a
	 * pointer to a character-class string -- b.c's own header comment says
	 * so -- written at b.c:222 (upstream 205) in cfoll() and read back as
	 * `(char *) f->re[i].lval' at b.c:683 (upstream 666) in cgoto().  A
	 * pointer through a four-byte field is 32 bits of a 64-bit address
	 * here, so /[abc]/ would match against a wild pointer.
	 *
	 * This is an in-memory table private to the DFA, not an on-disk record
	 * and not a struct crossing the shim seam, so widening it costs
	 * nothing but the eight bytes.
	 */
	long lval;
	fcell *lfollow;
} rrow;
