 ...
 *
 * and `(p) != NULL &&' is the whole change.  real_execute() opens with
 * `if (u == NULL) return(true);' -- so a null statement is EXPECTED and
 * harmless there -- but `isvalue(p)' is `(p)->ntype == NVALUE', which reads
 * offset 0 of p BEFORE that guard is ever reached.
 *
 ...
 * PROGRAM node, which program() below executes once per input record.  Bell
 * Labs' own null guard is the evidence that this was reached in 1985 and did
 * not fault.
 *
 * It did not fault because a VAX maps virtual 0: it is the first byte of crt0
 * in a ZMAGIC binary, so `(p)->ntype' read a word of program text, which is
 * not 1 (NVALUE), so the conditional fell to real_execute() and its guard
 * answered.  macOS leaves page 0 unmapped, so the same read is a SIGSEGV --
 * measured on `BEGIN{print 1}', which is the most common awk invocation there
 * is.  See src/cmd/awk/PORTING.md and PLAN.md S4i for the class.
 */
#define	execute(p)	((p) != NULL && isvalue(p) ? \
			    (Cell *)((p)->narg[0]) : real_execute(p))
