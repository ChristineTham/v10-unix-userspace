...
int	rtok;
/*
 * PORT: upstream `int rlxval'.  relex() stores a character value here for
 * CHAR and `(long) tostring(cbuf)' -- a HEAP POINTER -- for CCL and NCCL
 * (see :620 below), and primary() hands it to cclenter(), which takes a
 * char *.  Four bytes held both on a VAX; here the pointer needs eight.
 */
long	rlxval;
char	*prestr;
...
register Node *v;
{
	...
	switch(type(v)) {
		LEAF
			...
			/*
			 * PORT: upstream `= (int) right(v)'.  The comment at
			 * the head of this file says it: for a leaf, "right
			 * contains value OR POINTER TO VALUE" -- a character
			 * for CHAR, a character-class string for CCL/NCCL --
			 * and cgoto() below reads it back as
			 * `(char *) f->re[i].lval' for member().  struct
			 * rrow's lval is widened to long to match.  (Named
			 * rather than numbered because it is in this file:
			 * editing this comment moves the line it would cite.)
			 */
			f->re[(int) left(v)].lval = (long) right(v);
			for (i=0; i<line; i++)
				setvec[i] = 0;
			follow(v);
...
{
	extern long rlxval;	/* PORT: was int -- see the definition above */
	register int c;
	char cbuf[150];
	int clen, cflag;
	switch (c = *prestr++) {
		...
		case '[': 
			...
			for (;;) {
				if ((c = *prestr++) == '\\') {
					...
					cbuf[clen] = 0;
					rlxval = (long) tostring(cbuf);	/* PORT: (int) */
					if (cflag == 0)
						return CCL;
					else
						return NCCL;
