	...
	char **argv;
{
	...
	nlist("/unix", nl);
	/*
	 * PORT: was nl[0].n_type==0, i.e. "_proc".  On a VAX all nine of these
	 * symbols were present together or absent together, so testing the first
	 * was a sound proxy for "is there a namelist".  This port's kernel answers
	 * for some and not others -- _avenrun and _bootime exactly, the proc table
	 * and the VAX page tables not at all -- so the proxy no longer means what
	 * it meant, and it would fail uptime, which needs neither.  Name the two
	 * the path below actually reads.  The full-w path is unaffected: it opens
	 * /dev/mem next and says "No mem".  src/cmd/w/PORTING.md.
	 */
