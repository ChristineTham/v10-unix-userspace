...
int	plkflag = -1,	/* used to signal printf-like functions */
	slkflag = -1;	/* used to signal scanf-like functions */
#endif
...
#ifdef FMTARGS
	/*
	 * printf-like and scanf-like functions.
	 * "n" should be both the number of required arguments and
	 * the position of the format string in the arg list.
	 */
	if (plkflag > 0)
		if (n < plkflag)
			uerror ("declare the required arguments!");
		else {
			n = plkflag;
			lty |= LPF;
		}
...
#else
	/* PORT: fsname is STATIC, so on the first call f.fn is 0 -- and
	 * under FLEXNAMES it is a `char *' where the other arm (lint.h:58)
	 * is a `char fn[LFNM]' ARRAY, which zero-initialises to the empty
	 * string and cannot fault.  So the defect is SELECTED BY THE -D:
	 * upstream's own CFLAGS carry -DFLEXNAMES, and a VAX read virtual 0
	 * anyway -- ZMAGIC text begins 00 00 ... so address 0 IS the empty
	 * string, strcmp returns nonzero, and the `new one' arm is taken,
	 * which is what the first call is supposed to do.  Spelling the
	 * empty string reproduces that answer rather than merely avoiding
	 * the fault; quot's qcmp carries the identical construction for the
	 * identical reason.  macOS maps nothing at 0. */
	if( strcmp(s, fsname.f.fn? fsname.f.fn: "")) {
#endif
		/* new one */
#ifndef FLEXNAMES
	...
	for( p=argv[1]; argc>1 && *p; ++p ){
		...
		case 'S':
			...
			if (stmpfile = fopen (stmpname, "r+"))
				fseek (stmpfile, 0L, 2);
			else if (stmpfile = fopen (stmpname, "w"))
				/* see getlab() */
				fputs ("0123456789\n", stmpfile);
			else {
				uerror ("%s: can't open", stmpname);
				exit (1);
			}
			continue;
#endif
...
{
	if (c >= 0 && stmpfile)
		fputc (c & 0177, stmpfile);
	return;
}


/*
 * returned value should be at least 10.  see tmpopen().
 */

int
getlab()
{
	/* PORT: stmpfile is opened by the -S option and by NOTHING ELSE, and
	 * upstream's own rule for the two llib libraries runs `lint1 -v'
	 * with no -S -- so this ftell is handed a null FILE * every time a
	 * .ln library is built.  bycode(), the function immediately above,
	 * tests `c >= 0 && stmpfile' and this one tested nothing: the fix
	 * landing on one line while the line beside it keeps the
	 * assumption, from the other side.
	 *
	 * A VAX did not fault, because ftell only READS the FILE -- _cnt,
	 * _ptr, _base, _flag and then lseek(_file) -- and virtual 0 in a
	 * ZMAGIC binary is crt0's first bytes rather than an unmapped page.
	 * It returned a garbage long that nothing looked at.  The answer
	 * restored here is NOT that garbage: 10 is upstream's own, at
	 * macdefs:56, which is what getlab() expands to when FMTARGS is not
	 * defined at all, and the comment above this function already says
	 * the value must be at least 10. */
	if (stmpfile == NULL) return (10);
	return ((int) ftell (stmpfile));
}
#endif FMTARGS
