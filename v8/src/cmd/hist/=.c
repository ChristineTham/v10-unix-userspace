...
#define VECSIZE	512
/*
 * PORT: the cell of this vector is ONE MACHINE WORD, and it means two
 * different things at the two ends of the same array.  From matchvec upward
 * linesave() fills it with ftell() offsets, which prevline() hands back to
 * fseek(); from matchend downward prevline() fills it with savestr() results,
 * which it hands back to strcmp() through a (char *) cast.  Upstream spells
 * the cell `int' because a VAX pointer was four bytes, so both readings were
 * exact.
 *
 * Here the pointer half loses its top half.  Measured with the store and the
 * load printed side by side: savestr() returned 0x300001818 and the cell read
 * back 0x1818, and on the next run 0x104f1d818 became 0x4f1d818 -- the value
 * MOVES between runs, which is what says it is a real pointer being truncated
 * rather than garbage, and the remainder lands in the unmapped low pages, so
 * the strcmp faults.  `==' SIGSEGV'd the moment you typed `=' to step back a
 * line, which is the ordinary way to use the redo editor.
 *
 * So the fix is `widen the TYPE, not the uses' -- struct(1)'s VERT verdict and
 * bc's arena cell, reached a third time.  What a cell has to be is a machine
 * word, and no V8 type spells that; `long' is the one that is pointer-sized
 * here and was int-sized on a VAX, so the sources stay right for both.
 * Nothing else in the file changes: five declarations and one cast.
 *
 * vecsize is split onto its own line and stays `int'.  It is a count of cells,
 * never a cell, so widening it is not forced by the target.
 */
long *matchvec, *matchend, *lastmatch, *lastuniq;
int vecsize;

linesave(adr, i)
{
	if (matchvec == 0)
		matchend = matchvec = Malloc(long, vecsize = VECSIZE);
	if (i >= vecsize)
		matchvec = Realloc(long, matchvec, vecsize += VECSIZE);
	if (matchvec == 0)
		return;
	*matchend++ = adr;
}
...
FILE *f;
{
	...
	register char *hp; register long *ip;
	if (matchvec == 0)
		return;
	if (lastmatch == 0)
		lastuniq = matchend, lastmatch = --matchend;
	*--lastuniq = (long)savestr(linebuf);
	do {
