...
/*
 * PORT (LP64): asctime() AND gmtime() ADDED.  Both return pointers and
 * neither was declared, so the compiler took each result as an int and
 * truncated it -- `asctime(gmtime(&delta))+11' is two truncations in one
 * expression.  ctime and strspl were declared already, which is why only
 * the elapsed-time column was wrong; tests/v8ccom's rootfs-wide sweep for
 * truncated pointer-returning calls is what named it.
 */
char	*ctime(), *strspl(), *asctime();
struct tm *gmtime();
int	onintr();
