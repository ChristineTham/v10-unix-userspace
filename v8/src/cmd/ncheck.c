...
char *argv[];
{
	...
	while (--argc) {
		...
		case 'i':
			for(i=0; i<NB; i++) {
				/* PORT: argv[1] is the NULL that terminates
				 * the vector once -i's last number has been
				 * consumed, and atol() dereferences it at
				 * once.  On the VAX address 0 held a byte of
				 * the text segment, which was not a digit, so
				 * atol returned 0 and this loop broke; macOS
				 * leaves page 0 unmapped, so `ncheck -i 5'
				 * SIGSEGVs before it opens anything.  Same
				 * assumption as refer5.c's lookat(). */
...
register struct dinode *ip;
{
	...
	if((ip->di_mode & IFMT) != IFDIR) {
		if (sflg==0 || nxfile>=NB)
			return;
		if ((ip->di_mode & IFMT) == IFBLK || (ip->di_mode & IFMT) ==
			IFCHR || ip->di_mode & (ISUID | ISGID))
		{
			ilist[nxfile] = ino;
...
register struct dinode *ip;
{
	...
	for(i=0;; i++) {
		...
		pr:
			/* PORT: k is set only by the loop above, which runs
			 * only when an -i list is present, and the label is
			 * reached from BOTH sides -- so with -s and no -i,
			 * ilist[k] indexes by an uninitialised int.  Sign
			 * extended that is a +-4 GB offset from ilist; the VAX
			 * read something and macOS unmaps most of that range.
			 * Initialised to 0 rather than guarded, because
			 * ilist[0] is then 0 and kno == 0 is already skipped
			 * above, which is what the print meant to test. */
			hp = lookup(kno, 0);
			printf("%u\t", kno);
			if (sflg && kno == ilist[k])
				printf("%o\t", imodes[k]);
