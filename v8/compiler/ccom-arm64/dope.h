/*
 * Operator dope vector -- copied verbatim from ccom/vax/dope.h.
 *
 * Despite living in the machine-dependent directory, this table describes the
 * intermediate representation (operator names and LTYPE/UTYPE/BITYPE flags),
 * not the target machine.  It is reproduced here unchanged so the ARM64 build
 * does not reach into vax/, and so it can diverge later if a target ever needs
 * different operator properties.
 */
/* @(#) dope.h: 1.1 12/22/83				*/
int dope[ DSIZE ];
char *opst[DSIZE];
