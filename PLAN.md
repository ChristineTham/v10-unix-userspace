...
5. **`v8fs` as the third server** -- V8's own `alloc.c`, `iget.c`, `nami.c`,
   ...
   **What step 5f inherits is a list, not a guess.** The write half of the
   server (`Twrite`/`Tcreate`/`Tremove`/`Twstat`, all `EROFS` today, with the
   kernel underneath them written and tested); the eleven slotless syscalls,
   which refuse a mounted path with `EROFS` and become slots one at a time; and
   `chdir`, which is the one gap the client cannot close alone, because nothing
