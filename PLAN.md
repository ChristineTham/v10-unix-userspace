...
| 3C Wave C | in progress | **nroff and troff both run** (`wavec` 9/9). nroff fills and honours `.br`, `.ll`, `.ce`, `.sp`, `.na`; troff emits the device-independent stream for the 202 typesetter, with its tables compiled by `makedev` at build time. `tbl` compiles and links. See `src/cmd/troff/PORTING.md` |
