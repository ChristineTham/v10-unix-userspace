...
1. **Streams.** `streamio.c` is 1093 lines of portable multiplexing -- no MMU,
   ...
   **`spipe` is structurally impossible, and not by a small margin.** It is the
   ...
   diverges) and `exec` clears `fdtyp[]` (`vfs.c:451-466`).
...
5. **`v8fs` as the third server** -- V8's own `alloc.c`, `iget.c`, `nami.c`,
   ...
   *notice* is a property of the layout: under `-force_load` it is SIGSEGV, and
   ...
   The second reason needs no measurement and `shim/v8sys/vfs.c:450` had already
...
| 1b ARM64 back end | done | `v8ccom` 112/112 — arithmetic, control flow, pointers, arrays, globals, statics, recursion, structs, bitfields, floats, 12-argument calls |
| 1c driver | done | `v8cc` 14/14, `make rootfs` |
| 2a libv8sys | done | `v8sys` 191/191 — including signal *delivery*, not just numbering |
| 2b V8 libc | done | 89 objects, compiled by v8cc: stdio (incl. `%f`/`%e`/`%g`), the string family, malloc, ctype, qsort, getenv, the directory routines, `setjmp`/`longjmp`, perror and IEEE floats (`libv8c` 38/38) |
| 3A Wave A | done | **156 of 163** single-file commands in `usr/src/cmd` build, including `ls`. **All 48 imported into `src/cmd` are now real installed binaries in `rootfs/bin`** (every `.c` there but `cc`, which has its own rule), and `wavea` 124/124 runs the *installed* ones rather than temp-directory copies — the rule the rest of the tree already followed. That change is what surfaced seven commands (`ascii`, `bcd`, `cal`, `morse`, `ptx`, `units`, `vis`) that compiled but had never been shipped, and two data files (`/usr/lib/units`, `/usr/lib/eign`) that `units` and `ptx` read by absolute path — without which they answer "no table" and "Cannot open  file /usr/lib/eign" |
| 3B Wave B | done | The **Bourne shell** runs (`sh` 25/25) — see `src/cmd/sh/PORTING.md`. The file and process tools run too (`waveb` 26/26): `cp`, `mv`, `mkdir`, `rmdir`, `sed`, `ed`, `dc`, `factor`, `primes`, `tsort`. **38 multi-file command directories** compile and link, including `ps`, `w`, `df`, `tbl`, `qed`, `adb`, `yacc`, `man`, `diff3`, `dump`, `su`, `cron`, `compress` |
| 3C Wave C | **done** | **nroff, troff, tbl, eqn, pic, spell, man, grap and refer all run** (`wavec` 82/82). `tbl \| nroff` formats a table, `eqn \| nroff` sets an equation, `grap \| pic \| troff` draws a graph end to end, and `refer` resolves citations against an index its own `mkey`/`inv` built. eqn, pic and grap are built with **V8's own yacc and lex**, themselves compiled by v8cc. nroff fills and honours `.br`, `.ll`, `.ce`, `.sp`, `.na`; troff emits the device-independent stream for the 202 typesetter, with its tables compiled by `makedev` at build time. See the `PORTING.md` under `troff`, `tbl`, `pic`, `grap` and `refer` |
...
real bugs the compiler did not -- `last`'s truncated `asctime`/`gmtime`, five
...
**7c. What is left, in order of unlock:**
  ...
  - ~~**The crash probe's floor is 54 in prose and 160 in fact**~~ — **DONE.**
    ...
    **And that guard's first real run found a SECOND escape, `/dev/null`** — a
    ...
    absence from `ROOTFS_DEVSTD` was a gap; it is built now.

    **And building it BROKE it, which took a second sitting to see (task #78,
    now done).** With no rootfs node the path fell through to the host's device
    and was correct; what the check had caught was a program *poisoning* that
    fall-through. Materialising it moved the breakage from "after the first
    write" to "always", and blinded the detector, since a path already in the
    list is never new. Four faults measured — writes accumulated, `cat` handed
    the litter back, `test -c` was false, `ls -l` said `-rw-r--r--` — and the
    read is the sharp one, because `prog < /dev/null` is how a program is given
    empty input. Fixed as the **fifth filesystem type**, two operations:
    `t_path` returns the name unresolved so the host's device answers (Bell
    Labs' null is `dev/mem.c:68` and `:156`, which is what Darwin's does), and
    a `t_stat` that overrides `st_rdev` alone — because the two machines agree
    the device is 3,2 and disagree about the PACKING, Darwin at bit 24 and V8
    at bit 8, so `stat_translate`'s `& 0xffff` deletes the major rather than
    narrowing it. Guarded in `tests/v8sys` (mechanism), `tests/wavea`
    (installed binaries) and the probe itself (the node's size across the
    sweep, since the path list can no longer see it).

    So a containment check doubles as a **completeness check on
  ...
  - **Strip `04000`/`02000` in `pt_chmod`.** A package doing `chmod 4755`
    currently sets a real setuid bit on a real Mach-O. Harmless while the file
    is user-owned and an escalation the moment anything is root-owned, and it
