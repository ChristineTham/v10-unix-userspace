	...
	char *pathname;
{
	...
	*pnptr = '\0';
	...
	for (;;) {
		stat(CURDIR, &d);
		fprintf(stderr, "LEVEL cwd ino=%ld dev=%ld\n", (long)d.st_ino, (long)d.st_dev);
		...
		fstat(dirp->dd_fd, &dd);
		fprintf(stderr, "  parent ino=%ld dev=%ld\n", (long)dd.st_ino, (long)dd.st_dev);
		if (d.st_dev == dd.st_dev) {
			...
			do {
				if ((dir = readdir(dirp)) == NULL) {
					closedir(dirp);
					fprintf(stderr, "  FAIL scanning for ino=%ld\n", (long)d.st_ino);
